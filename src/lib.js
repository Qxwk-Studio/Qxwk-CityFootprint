...
// 用 Bearer token 去通行证 /api/me 验证，返回 {userId, nickname, color, email} 或 null
async function getPassportUser(request) {
  ...
  try {
    ...
    return { userId: u.userId, nickname: u.nickname, color: u.color, email: u.email };
  } catch {
    return null;
  }
}

// 通行证用户 -> 本地 CF 用户：首次自动建（颜色同步通行证），返回 {id, nickname, color, isAdmin, email} 或 null
async function resolveViewer(DB, request) {
  ...
  return u ? { id: u.id, nickname: u.nickname, color: u.color, isAdmin: !!u.is_admin, email: p.email } : null;
}
